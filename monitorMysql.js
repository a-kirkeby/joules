...
const pageVisit = async (browser, websiteId, tenantId, siteName, url, measurementId, depth = 0, maxDepth = -1) => {
    ...
    page.on('response', async response => {
        ...
        try {
            ...
            //console.log('done processing response', requestUrl, {decodedBodySize, requestPayloads})
        } catch (error) {
    ...
    perfEntries.map(x => {
        ...
        const payload = requestPayloads.find(y => y.requestUrl == x.pageUrl)
        if (!payload) return
...
const runSite = async (siteId) => {
    ...
    console.log('inserted measurement', { measurementId, name, url })
